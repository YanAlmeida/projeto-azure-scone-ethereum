resource "azurerm_linux_virtual_machine" "vm_sgx" {
  ...
  custom_data = base64encode(<<-CUSTOM_DATA
    ...
    sudo docker run -d -e BLOCKCHAIN_ADDRESS="${var.public_ip_blockchain}:8545" -e CONTRACT_ABI="${var.contract_abi}" -e CONTRACT_ADDRESS="${var.contract_address}" -e ACCOUNT_INDEX="${var.account_index} ${var.dockerhub_image_sgx_untrusted}"
    CUSTOM_DATA
  )
