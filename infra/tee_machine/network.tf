resource "azurerm_network_interface" "ni_sgx" {
  ...
  ip_configuration {
    ...
    public_ip_address_id          = var.generate_public_ip[var.account_index] ? azurerm_public_ip.public_ip_sgx[0].id : null
  }
}

resource "azurerm_public_ip" "public_ip_sgx" {
  count               = var.generate_public_ip[var.account_index] ? 1 : 0
